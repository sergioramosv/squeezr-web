...
/* ── Diagonal light beams — Raycast style in green ── */
const BEAMS = [
  { left: "-5%",  width: "22%", opacity: 0.55, color: "#052e16" },
  { left: "10%",  width: "26%", opacity: 0.85, color: "#166534" },
  { left: "28%",  width: "20%", opacity: 0.95, color: "#15803d" },
  { left: "43%",  width: "28%", opacity: 0.80, color: "#22c55e" },
  { left: "62%",  width: "22%", opacity: 0.70, color: "#166534" },
  { left: "76%",  width: "20%", opacity: 0.55, color: "#052e16" },
  { left: "88%",  width: "18%", opacity: 0.40, color: "#14532d" },
];

function BeamBackground({ intensity = 1 }: { intensity?: number }) {
  return (
...
export function HeroSection() {
  ...
  useEffect(() => {
    const t = [
      setTimeout(() => { setPhase("big"); setBeamIntensity(1); },     200),
      setTimeout(() => setPhase("compress"),                           850),
      setTimeout(() => { setPhase("impact"); setBeamIntensity(1.6); }, 850 + PRESS_MS),
      setTimeout(() => { setPhase("done");   setBeamIntensity(1); },   850 + PRESS_MS + 400),
    ];
  ...
  return (
    <section className="relative w-full h-screen -mt-16 bg-[#020a04] overflow-hidden flex flex-col items-center justify-center">
      ...
      <motion.div
        ...
        animate={{ opacity: beamIntensity }}
        transition={{ duration: 0.3, ease: "easeOut" }}
      >
        <BeamBackground />
      </motion.div>

      {/* ── Film grain on top of beams ── */}
      <div
        className="grain absolute pointer-events-none z-20"
        style={{ inset: "-20%", width: "140%", height: "140%", opacity: 0.08 }}
      >
      ...
      <AnimatePresence>
        {phase !== "idle" && (
          <motion.div
            key="text"
            className="relative z-10 flex items-baseline select-none"
            style={{ gap: "0.01em" }}
            ...
            animate={
              phase === "big"
                ...
                : isCompressing
                  ? { scale: [4.5, 4.5, 0.88, 1], x: [0, 0, 0, -6, 6, -3, 3, 0], opacity: 1 }
                  : { scale: 4.5, opacity: 1, x: 0 }
            }
            transition={
              phase === "big"
        ...
        {phase === "done" && (
          ...
          >
            ...
            <a
              ...
              target="_blank"
              rel="noopener"
              className="px-6 py-3 border border-white/20 text-neutral-300 font-bold text-sm rounded-xl hover:border-white/40 hover:text-white transition-colors flex items-center gap-2"
            >
